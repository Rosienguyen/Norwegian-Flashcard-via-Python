...
def select_card():
    global index
    index = simpledialog.askinteger("Select Card", "Enter the index of the card you want to select:")
    if index is not None:
        if index >= len(to_learn):
            index = len(to_learn) - 1
        elif index < 0:
            index = 0
        current_card = to_learn[index]
        cur_text = 'NO'
...
back_image = PhotoImage(file='./images/arrow_left_resized.png')
back_button = Button(image = back_image, text = 'back', command = prev_card)
back_button.grid(row = 1, column = 0, sticky = 'E' )

select_button = Button(text="Select Card", command=select_card)
select_button.grid(row=2, column=0, columnspan=4)
